/* Генерирует два демо-«снимка» (плейсхолдеры с глифом снаряжения)
   в www/sirocco.jpg и www/reverso.jpg. seed() ссылается на них по имени.
   Только для веб-прототипа (на устройстве старт пустой). */
...
async function jpg(svg,name){
  const p=path.join(WWW,name);
  await sharp(Buffer.from(svg)).resize(480,600).jpeg({quality:58}).toFile(p);
  return (fs.statSync(p).size/1024|0)+'КБ';
}
(async()=>{
  const s=await jpg(card('#F5EBD8','#D9C79B','#7A5610',HELMET,'Petzl Sirocco'),'sirocco.jpg');
  const r=await jpg(card('#E8ECEE','#B4C0C7','#33414B',BELAY,'Petzl Reverso'),'reverso.jpg');
  console.log('демо-фото: sirocco.jpg '+s+', reverso.jpg '+r);
